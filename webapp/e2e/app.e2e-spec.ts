...
describe('Starting tests for interlace-app', function() {
  ...
  it('website title should be interlace-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('interlace-app');
    })
  });

  it('network-name should be sardex-open-network@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('sardex-open-network@0.0.1.bna');
    });
  });

  it('navbar-brand should be interlace-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('interlace-app');
    });
  });
    ...
    it('SysAccount table should have 5 columns',() => {
      page.navigateTo('/SysAccount');
    ...
    it('CCAccount table should have 8 columns',() => {
      page.navigateTo('/CCAccount');
      element.all(by.css('.thead-cols th')).then(function(arr) {
    ...
    it('Individual table should have 7 columns',() => {
      page.navigateTo('/Individual');
      element.all(by.css('.thead-cols th')).then(function(arr) {
