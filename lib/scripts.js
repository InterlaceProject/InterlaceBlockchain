...
function CreditTransfer(tx) {
	//get the sender
	var sender = tx.senderAccount;
	console.log("sender is", sender);
	//get the recipient
	var recipient = tx.recipientAccount;
	...
	if( tx.amount <= 0 ) {
		throw  "Transfer amount cannot be less than or equal to zero";
	}
	...
	return getAssetRegistry('org.s.network.Account')
		.then(function (accountAssetRegistry) {
			...
			if( sender.UnitID !== recipient.UnitID) {
 ...
 * @transaction
 */
function CreditTransfer_OLD(tx) {
	//get the sender
