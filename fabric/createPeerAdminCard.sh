...
#eh: removed PRIVATE_KEY and CERT => those are preconfigured in _config.sh

if [ "${NOIMPORT}" != "true" ]; then
    CARDOUTPUT=/tmp/${PEER_ADMIN_CARD}
else
    CARDOUTPUT=${PEER_ADMIN_CARD}
fi
...
if [ "${NOIMPORT}" != "true" ]; then
    if "${HL_COMPOSER_CLI}"  card list -c ${PEER_ADMIN_CARD} > /dev/null; then
        "${HL_COMPOSER_CLI}"  card delete -c ${PEER_ADMIN_CARD}
    fi

    "${HL_COMPOSER_CLI}"  card import --file /tmp/${PEER_ADMIN_CARD} 
    "${HL_COMPOSER_CLI}"  card list
    echo "Hyperledger Composer PeerAdmin card has been imported, host of fabric specified as '${HOST}'"
    rm /tmp/${PEER_ADMIN_CARD}
else
